...
**Session-health gate.** This pipeline writes DDL and reads/writes the workspace, so it **cannot run in a read-only or connection/MCP session**. STOP - do not create anything - if `CURRENT_ROLE()` looks read-only or service-bound (ends `_READONLY`, or an MCP/service role like `CLAUDE_MCP_*`) or `CURRENT_WAREHOUSE()` is a connection warehouse (e.g. `*_MCP_*`): the session is bound to the wrong connection; tell the user to re-open the setup in the **interactive Snowsight Workspace as the AGENTS.md role** (with the AGENTS.md warehouse active), then resume. This is **not a one-time check**: a chat reload or page refresh can silently re-bind the session to a different (read-only) connection and break workspace file access - **re-verify role + warehouse before any DDL or file read on every resumed turn**. If DDL or a file read suddenly fails with an auth / token-expiry / permission / `I/O` error mid-run, treat it as a **degraded session → STOP and surface it; never continue, scan, or deploy from memory.**
...
8. **Run the `$sis` pre-deploy scan across ALL app files as a final confirmation** - first **re-open `$sis` itself** (don't scan from a remembered checklist; if its text isn't in context this turn - e.g. trimmed earlier - load it before scanning), then **re-read every app file from disk now, never scanning from memory** (a remembered scan certifies code you didn't look at and can't catch a `NameError`/`ImportError`). The scan's static-resolution items (every imported name + every `clear_caches()` loader resolves to a real definition; no raw-`Row` `.get()`; write-once side effects) are what catch the import/name/attribute errors a static read must find. If you applied the item-2 rules it reports 0 failures - that's the target; >0 means a rule was skipped during generation (fix, **re-read**, re-scan). ⚠️ Do NOT deploy on any FAIL. **Writing the app files this turn does NOT substitute for re-reading them** (generation ≠ verification) - re-read each from disk now, including any file trimmed from context (re-read it, never reconstruct it from memory). The **"consecutive reads / possible loop" warning during this mandatory re-read is an EXPECTED false positive** - re-read every file regardless; do not stop short and fill the rest from memory. **If you cannot re-read the app files from disk this turn** (read-only/degraded session, broken workspace mount, file-not-found, `I/O`/auth errors), you **cannot run the scan OR the UX-conformance gate** - **STOP and do not deploy**; report the access failure and its remediation (re-open in a healthy interactive session - Step 1a). **Never certify or deploy from memory or from a prior turn's reads**.
