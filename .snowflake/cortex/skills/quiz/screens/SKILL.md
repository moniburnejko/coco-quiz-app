...
21. **Batch feedback.** FAIL if batch generation doesn't run under an `st.spinner` AND fire an `st.toast` AND show a transient **plain-text** "Added N …" line (NOT a badge); FAIL if batch generation renders any extra explanatory caption - the toast + plain-text line are the whole feedback.
