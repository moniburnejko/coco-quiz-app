...
| `_config.py` | `EXAM_NAME`, `EXAM_CODE`, `CORTEX_MODEL`, `MODEL_OPTIONS` (per-call-group model picklist - `$cortex`), `PASS_THRESHOLD`, `DIFFICULTY_GUIDE`, color + `RESPONSE_FORMATS` constants |
...
Navigation is native multipage (`st.Page` + `st.navigation` in `main.py`); `st.session_state` is shared across pages (initialized once in `main.py`). Full flow/state/write-back contracts → `screens/`.
