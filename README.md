...
- **A learning loop, not just a quiz**: Socratic hints before answering (never spoil), an on-demand AI explanation after (for correct answers too) with a 🔬 deep dive on one option, and an AI debrief of round patterns. Two optional add-ons extend it: Comparison (compare two options side-by-side) and a Remedial Round (re-test your wrong answers when you fail).
...
- **Optional add-ons** (five, off by default): exam-simulation mode, flashcards (atomic recall cards + Leitner spaced repetition), AI study recommendations, concept comparison, and a remedial round (re-test your wrong answers) (`$quiz/features`). Five further ideas (Quick Stats, Smart Review, Achievement Badges, Misconception Analysis, Flag a Question) are designed-but-deferred in [docs/future-features.md](docs/future-features.md).
