...
The **Review** page (separate sidebar pill) has tabs: **Wrong Answers** (filtered `QUIZ_REVIEW_LOG` history with domain and date-range filters) and **Learning Dashboard** (session trends - score-per-session line chart from `QUIZ_SESSION_LOG`, error distribution from `QUIZ_REVIEW_LOG` grouped by domain, readiness score against the 75% threshold).
...
(A fifth table, `QUIZ_CONFIG`, holds runtime app configuration for the Admin page - key-value, outside the ER above; it also stores `grounding_mode`, fixed at setup. A transient `_DOC_CONTENT` holds the parsed PDF during setup and is dropped afterward.)
